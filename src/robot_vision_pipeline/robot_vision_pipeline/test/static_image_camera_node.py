...
Parameters:
  ...
  fx, fy, cx, cy:  camera intrinsics (default fx=fy=615, cx=320, cy=240 for 640x480)
"""
...
class StaticImageCameraNode(Node):
    def __init__(self) -> None:
        ...
        if self._cx == 320.0 and self._cy == 240.0:
            if self._width != 640 or self._height != 480:
                pass
    ...
    def _load_image(self, image_path: str) -> np.ndarray:
        if not image_path:
            self.get_logger().error("image_path is empty or image file does not exist")
            return None
